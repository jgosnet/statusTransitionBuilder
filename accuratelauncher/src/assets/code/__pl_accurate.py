...
class AccurateConfiguration():
    """ Stores and prepares dynamic configuration for multiple work-orders: inputspec and video_data set """
    def __init__(self, input_specs=None, video_data=None):
        self.input_specs = input_specs or {}
        self.video_data = video_data or {}

    @classmethod
    def from_wo_list(cls, wo_list):
        """ First wo passed will be the "primary" """
        input_specs = {}
        video_data = {}
        for index, wo in enumerate(wo_list):
            reference = f"secondary_{index}"
            if index == 0:
                reference = "primary"

            input_specs.update(wo.get_input_specs())
            video_data.update({reference: wo.get_video_data()})

        return cls(input_specs=input_specs, video_data=video_data)
